...
export const getSalonSpaById = async (id: number) => {
    try {
        const { data, error } = await supabase.from('salon_spas').select('*').eq('id', id);
        if (error) {
            return {
                success: false,
                message: error.message,
...
export const updateSalonSpaById = async ({ id, payload }: {
    id: number,
    payload: any
}) => {
...
export const deleteSalonSpaById = async (id: number) => {
    try {
        const { data, error } = await supabase.from('salon_spas').delete().eq('id', id);
        if (error) {
            return {
                success: false,
                message: error.message,
